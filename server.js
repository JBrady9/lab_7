...
app.get('/player_info', (req, res) => {
  ...
  app.get('/player_info/post', (req, res) => {
	  const players = 'SELECT * FROM football_players';
	  const player_Id = "SELECT * FROM football_players WHERE name=\'" + req.query.player_choice + "\';";
	  const games = "select count(football_games.*) from football_players inner join football_games on football_players.id = any(football_games.players) where football_players.name=\'" + req.query.player_choice + "\';";
	  ...
	  db.task('get-everything', task => task.batch([
		...
		.then(info => {
		  info[1].games = info[2].count;
		  res.render('pages/player_info', {
			my_title: info[1].name,
			players: info[0],
			player_info: info[1],
		  })
		})
